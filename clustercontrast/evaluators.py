...
def extract_cnn_feature(model, inputs,mode):
    ...
    #
    outputs, outputs_pa = model(inputs, inputs, modal=mode)
    outputs = outputs.data.cpu()
    outputs_pa = outputs_pa.data.cpu()
    return outputs, outputs_pa
...
def extract_features(model, data_loader, print_freq=50,flip=True,mode=0):
    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, _, _) in enumerate(data_loader):
            ...
            outputs, outputs_pa = extract_cnn_feature(model, imgs, mode)
            flip = fliplr(imgs)
            # print(flip)
            # outputs_flip = extract_cnn_feature(model, flip,mode)
            outputs_flip, outputs_pa_flip = extract_cnn_feature(model, flip, mode)
            ...
            for fname, outputs_pa, outputs_pa_flip, pid in zip(fnames, outputs_pa, outputs_pa_flip, pids):
                features_pa[fname] = (outputs_pa.detach() + outputs_pa_flip.detach()) / 2.0
    ...
    return features, features_pa, labels


#####################
def extract_features_vit(model, data_loader, print_freq=50, mode=0):
    model.eval()
    batch_time = AverageMeter()
    data_time = AverageMeter()
